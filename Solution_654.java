...
class Solution_654 {
    ...
    public TreeNode constructMaximumBinaryTree(int[] nums) {
        return constructTreeInternal(nums, 0, nums.length);
    }
    ...
    private TreeNode constructTreeInternal(int[] nums, int start, int end) {
        if(end - start <= 0) {
            return null;
        }
        else {
            ...
            rootNode.left = constructTreeInternal(nums, start, maxIndex);
            rootNode.right = constructTreeInternal(nums, maxIndex + 1, end);
            return rootNode;
        }
    }

    public static void main(String args[]) { 
        Solution_654 s = new Solution_654();
        ...
        TreeNode b = s.constructMaximumBinaryTree(a);
